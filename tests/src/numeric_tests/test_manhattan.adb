--  Test cases for Manhattan distance function
...
package body Test_Manhattan is
   ...
   function Long_Float_Manhattan is new
     Distance.Numeric.Manhattan_Generic
   ...
   begin
      return AUnit.Format ("Manhattan Distance Tests");
   end Name;
   ...
   begin
      Register_Routine (T, Test_Grid_Distance'Access, "Grid Distance");
   ...
   procedure Test_Grid_Distance (T : in out AUnit.Test_Cases.Test_Case'Class) is
      pragma Unreferenced (T);
      V1       : constant Float_Vector := (0.0, 0.0);
      V2       : constant Float_Vector := (3.0, 4.0);
      Distance : constant Float := Float_Manhattan (V1, V2);
   begin
      Assert
        (abs (Distance - 7.0) < 0.0001, "Grid distance (0,0) to (3,4) should be 7.0, got" & Float'Image (Distance));
   end Test_Grid_Distance;
   ...
   procedure Test_Zero_Distance (T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Distance : constant Float := Float_Manhattan (V, V);
   begin
   ...
   procedure Test_High_Dimensional (T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Distance : constant Float := Float_Manhattan (V1, V2);
      Expected : constant Float := 10.0;  --  sum of 10 * |1-0|
   begin
      Assert (abs (Distance - Expected) < 0.0001, "10D unit distance should be 10.0, got" & Float'Image (Distance));
   ...
   procedure Test_Long_Float_Instantiation (T : in out AUnit.Test_Cases.Test_Case'Class) is
      ...
      Distance : constant Long_Float := Long_Float_Manhattan (V1, V2);
      Expected : constant Long_Float := 2.0;  --  |1-0| + |1-0|
   begin
      Assert (abs (Distance - Expected) < 0.0001, "2D unit grid should be 2.0, got" & Long_Float'Image (Distance));
